      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        ...
        {reportData && (
          <div className="space-y-6 pb-6">
            ...
            {/* Agentic Insights */}
            {reportData.agenticInsights && (
              <div className="bg-gradient-to-br from-purple-900/40 to-indigo-900/40 border border-purple-500/30 rounded-lg p-4">
                <h3 className="text-purple-300 font-bold text-sm uppercase mb-3 flex items-center gap-2">
                  <span>🧠</span>
                  <span>Agent Reasoning Chain</span>
                  <span className="text-xs font-normal text-purple-400/60">({reportData.agenticInsights.roundsRun} rounds · {reportData.agenticInsights.signalsDetected} signals)</span>
                </h3>
                <div className="space-y-4">
                  {reportData.agenticInsights.agentReasoning?.map((reasoning: any, idx: number) => (
                    <div key={idx} className="bg-recon-navy/60 border border-purple-500/20 rounded p-3">
                      <div className="flex items-start gap-2 mb-2">
                        <span className="text-purple-400 font-bold text-sm">R1 Signal {idx + 1}:</span>
                        <span className="text-white text-sm">{reasoning.signal}</span>
                      </div>
                      <div className="flex items-start gap-2 mb-2 pl-4">
                        <span className="text-purple-300/60 text-xs">→ Hypothesis:</span>
                        <span className="text-recon-grey text-xs">{reasoning.hypothesis}</span>
                      </div>
                      <div className="flex items-start gap-2 mb-2 pl-4">
                        <span className="text-purple-300/60 text-xs">→ Follow-up Query:</span>
                        <span className="text-purple-200 text-xs font-mono">{reasoning.followupQuery}</span>
                      </div>
                      {reasoning.discovered && (
                        <div className="flex items-start gap-2 pl-4 pt-2 border-t border-purple-500/10">
                          <span className="text-green-400 text-xs">✓ Discovered:</span>
                          <span className="text-green-300/80 text-xs">{reasoning.discovered}</span>
                        </div>
                      )}
                    </div>
                  ))}
                  {reportData.agenticInsights.intelligenceUpgrade && (
                    <div className="bg-purple-900/30 border border-purple-400/20 rounded p-3 mt-4">
                      <div className="text-purple-300 text-xs font-semibold mb-1">Intelligence Upgrade</div>
                      <div className="text-white text-sm">{reportData.agenticInsights.intelligenceUpgrade}</div>
                    </div>
                  )}
                </div>
              </div>
