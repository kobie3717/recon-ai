...
function handlePrint(reportData: any) {
  const domain = reportData?.meta?.domain || 'mcp';
  const date = reportData?.meta?.analysisDate || new Date().toISOString().split('T')[0];
  const prev = document.title;
  document.title = `Recon MCP Intelligence - ${domain} - ${date}`;
  window.print();
      ...
      <div className="bg-recon-navy/80 px-6 py-4 border-b border-orange-500/30 flex items-center justify-between">
        ...
        {reportData && (
          <button
            ...
            className="text-recon-grey hover:text-white text-sm flex items-center gap-1.5 px-3 py-1 rounded border border-orange-500/30 hover:border-orange-400/50 transition-colors"
          >
            ↓ PDF
          </button>
        )}
